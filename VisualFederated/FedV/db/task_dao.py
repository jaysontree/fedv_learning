...
from db.db_models import DB, Task, Job, TaskResult,TaskProgress,is_local
from utils.core_utils import current_datetime,get_commit_id
import datetime
import json
...
class TaskDao(Logger):
    ...
    def save_task_result(self,task_result: dict,component_name: str,type: str):
        ...
        try:
            ...
            with DB.connection_context():
                ...
                jobs = Job.select().where(Job.job_id == task.job_id)
                if len(jobs) != 0:
                    job = jobs[0]

                model.name = f"{job.name}-Model"
    ...
    def update_serving_model(self,type: str, source_component: str):
        """
        ...
        try:
            ...
            with DB.connection_context():
                ...
                model.source_component = source_component
                model.save()
                return model
        except Exception as e:
    ...
    def get_flow_id(self):
        try:
            ...
            self.exception(e)
            logging.error(f"can not get flow id of task {self._task_id}, error is {e}")